...
def run_stress_tests():
    ...
    cases = {
        ...
        "Broken Timestamps": [
            TranscriptSegment(start=5.0, end=2.0, text="Backward time", words=[])
        ],
...
def run_ablation():
    ...
    clip = [
        TranscriptSegment(
            start=0.0, end=1.0, text="You won't believe this secret trick!", words=[]
        ),
    ...
    for judge_name in judges_backup.keys():
        ...
        try:
            ...
            score_diff = (
                abs(full_report.scroll_stop_prob - ablated_report.scroll_stop_prob)
            ...
            if score_diff < 0.01:
                log.warning(
                    f"Judge '{judge_name}' ABLATION FAILED: No significant impact on final scores (diff={score_diff}). Redundant?"
                )
            else:
...
def test_behavioral_optimization():
    ...
    clip = [
        ...
        TranscriptSegment(
            start=4.0, end=5.0, text="You won't believe this secret trick!", words=[]
        ),  # The hook after 1s dead air
