...
def publish_clip(clip_name: str, background_tasks: BackgroundTasks) -> dict[str, str]:
    ...
    def _upload() -> None:
        try:
            ...
            desc = "Generated by Shorts Clipper Autopilot."
            upload_short(str(path), title=title, description=desc, tags=["shorts", "viral"])
            logger.info("✅ Clip %s uploaded to YouTube successfully!", clip_name)
