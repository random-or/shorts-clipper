...
def _tracked_popen_init(self, *args, **kwargs):
    ...
    if job_id:
        with _proc_lock:
            ...
            logger.info(
                "Registered subprocess PID %s for active job %s", self.pid, job_id
            )
...
def cancel_job(job_id: str) -> None:
    ...
    logger.info(
        "Job %s cancelled in SQLite DB; worker will handle termination.", job_id
    )
...
def publish_clip(
    ...
    title = meta.get(
        "title",
        clip_name.replace(".mp4", "").replace("_", " ")[:90] + " #shorts #viral",
    )
    desc = meta.get(
        "description", "Generated by Shorts Clipper Autopilot. #shorts #viral"
    )
...
def connect_youtube(request: Request) -> dict[str, str]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate OAuth URL: {exc}"
        ) from exc
...
def disconnect_youtube() -> dict[str, Any]:
    ...
    if token_path.exists():
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to delete token: {e}"
            ) from e
...
def save_settings(payload: SettingsModel) -> dict[str, str]:
    ...
    try:
        ...
        env_lines.append(
            f"SHORTS_ENABLE_GPU={'true' if payload.enable_gpu else 'false'}"
        )
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="vanguard_scout_") as temp_dir:
            ...
            if not rough_segments:
                logger.info(
                    "No native English subtitles. Downloading 5-min audio sample..."
                )
...
def get_video_details(payload: TranscriptRequest) -> dict[str, str]:
    ...
    try:
        ...
        logger.warning(
            "Failed to fetch video details via yt-dlp: %s. Using placeholder.", e
        )
        return {
            "title": "YouTube Video",
