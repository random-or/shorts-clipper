...
TIME_PATTERN = re.compile(r"\b(?:seconds?|minutes?|hours?|days?|weeks?|months?|years?|today|yesterday|tomorrow)\b", re.IGNORECASE)
...
class FeatureExtractor:
    ...
    def extract(cls, segments: list[TranscriptSegment]) -> FeatureSet:
        ...
        for s in segments:
            ...
            # Semantic extraction
            if any(w in s_text for w in ["um", "uh", "hello guys", "welcome back", "so yeah", "anyway"]):
                is_dead_narrative = True
                state = NarrativeState.SETUP
            elif "you won't believe" in s_text or "secret" in s_text or "mysterious" in s_text or "insane" in s_text:
                is_hook = True
                hook_hits += 1
            ...
            semantic_segments.append(SemanticSegment(
                ...
                state=state,
                is_hook=is_hook,
                is_dead_narrative=is_dead_narrative
            ))
    ...
    def _empty_feature_set(cls) -> FeatureSet:
        return FeatureSet(
            ...
            emotion_hits=0, viral_hits=0, hook_hits=0, story_arc_markers=0, contradictions=0,
            numbers=0, money_references=0, time_references=0, pause_density=0.0,
            speaker_changes=0, sentiment=0.0, repetition=0, visual_dependency_markers=0,
            raw_words=[], duration=0.0, start_time=0.0, end_time=0.0, semantic_segments=[]
        )
